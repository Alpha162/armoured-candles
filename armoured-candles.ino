...
bool beginHttpForUrl(HTTPClient& http, const String& url) {
    ...
    if (url.startsWith("https://")) {
        ...
        // Avoid setFingerprint() because some ESP32 cores expose NetworkClientSecure
        // without that API, which causes compile failures.
        if (REMOTE_OTA_TLS_FINGERPRINT[0] != '\0') {
            Serial.println("Remote OTA: configured TLS fingerprint is ignored on this build (no setFingerprint support).");
        }
        secure.setInsecure();
